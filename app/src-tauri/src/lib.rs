...
fn write_text_file(path: String, content: String) -> Result<(), String> {
    ...
    // On other platforms, keep the path as-is
...
fn find_base_folder_from_log() -> Result<Option<PathBuf>, String> {
    ...
    let re = Regex::new(
        r#"[Launcher::launch] Launching Star Citizen (?:LIVE|PTU|HOTFIX) from \((.*?)\)"#,
    )
...
fn find_available_versions(base_folder_path: String) -> Result<Vec<String>, String> {
    ...
    for entry in entries {
        ...
        if path.is_dir() {
            ...
            let bin_dir = path.join("Bin64");
            let launcher_exe = path.join("StarCitizen_Launcher.exe");
            let game_exe = bin_dir.join("StarCitizen.exe");
